...
fn socks5_connect(
    proxy: &ProxyTestRequest,
    target_host: &str,
    target_port: u16,
) -> Result<TcpStream, String> {
    validate_proxy_request(proxy)?;
    let mut stream = TcpStream::connect((proxy.host.as_str(), proxy.port))
        .map_err(|e| format!("Não foi possível conectar à proxy SOCKS5: {e}"))?;
    ...
    match target_host.parse::<IpAddr>() {
        ...
        Err(_) => {
            ...
            if bytes.is_empty() || bytes.len() > 255 {
                return Err("Destino SOCKS5 inválido.".into());
            }
    ...
    if head[0] != 0x05 || head[1] != 0x00 {
        return Err(format!(
            "A proxy SOCKS5 recusou o destino (código {}).",
            head[1]
        ));
    }
    match head[3] {
...
fn test_socks_ip(proxy: &ProxyTestRequest) -> Result<String, String> {
    ...
    let body = text
        .split_once("\r\n\r\n")
        .map(|(_, body)| body)
        .ok_or("Resposta inválida no teste de IP.")?;
    let payload: serde_json::Value = serde_json::from_str(body.trim())
        .map_err(|e| format!("Resposta de IP inválida: {e}"))?;
    payload
        .get("ip")
        .and_then(serde_json::Value::as_str)
        .map(str::to_string)
        .ok_or("IP não encontrado na resposta.".into())
}
...
async fn execute_proxy_test(request: ProxyTestRequest) -> ProxyTestResult {
    ...
    if request.protocol == "socks5" {
        ...
        return match result {
            Ok(Ok(ip)) => ProxyTestResult {
                ok: true,
                ip: Some(ip),
                latency_ms: Some(started.elapsed().as_millis() as u64),
                error: None,
            },
            Ok(Err(error)) => ProxyTestResult {
                ok: false,
                ip: None,
                latency_ms: None,
                error: Some(error),
            ...
            Err(error) => ProxyTestResult {
                ...
                ip: None,
                latency_ms: None,
                error: Some(format!("Falha no teste SOCKS5: {error}")),
            },
        };
    }
    ...
    let mut proxy = match reqwest::Proxy::all(&proxy_url) {
        ...
        Err(error) => {
            return ProxyTestResult {
                ...
                ip: None,
                latency_ms: None,
                error: Some(format!("Proxy inválida: {error}")),
            }
        }
    };
        ...
        .build()
    {
        ...
        Err(error) => {
            return ProxyTestResult {
                ...
                ip: None,
                latency_ms: None,
                error: Some(format!("Não foi possível preparar a conexão: {error}")),
            }
        }
    };
    let response = match client.get("https://api.ipify.org?format=json").send().await {
        ...
        Err(error) => {
            return ProxyTestResult {
                ...
                ip: None,
                latency_ms: None,
                error: Some(format!("A proxy não respondeu: {error}")),
            }
        }
    };
    if !response.status().is_success() {
        return ProxyTestResult {
            ...
            ip: None,
            latency_ms: None,
            error: Some(format!("Teste retornou HTTP {}.", response.status())),
        };
    }
    let payload: serde_json::Value = match response.json().await {
        ...
        Err(error) => {
            return ProxyTestResult {
                ...
                ip: None,
                latency_ms: None,
                error: Some(format!("Resposta de IP inválida: {error}")),
            }
        }
    };
    let ip = payload
    ...
    ProxyTestResult {
        ...
        error: if ok {
            ...
            Some("IP não encontrado na resposta.".into())
        },
    }
}
...
fn read_http_request(stream: &mut TcpStream) -> Result<(Vec<u8>, usize), String> {
    ...
    loop {
        ...
        if count == 0 {
            return Err("Conexão local encerrada antes do cabeçalho HTTP.".into());
        }
        ...
        if data.len() > 64 * 1024 {
            return Err("Cabeçalho HTTP local excedeu o limite.".into());
        }
    }
}
...
fn start_socks_bridge(proxy: ProxyTestRequest) -> Result<(u16, Arc<AtomicBool>), String> {
    let listener = TcpListener::bind(("127.0.0.1", 0))
        .map_err(|e| format!("Não foi possível abrir a ponte local: {e}"))?;
        ...
        while !stop_thread.load(Ordering::Relaxed) {
            match listener.accept() {
                Ok((stream, _)) => {
                    ...
                    thread::spawn(move || {
                        let _ = handle_socks_bridge_client(stream, &request);
                    });
                }
...
fn push_if_exists(items: &mut Vec<BrowserInfo>, name: &'static str, path: PathBuf) {
    if path.is_file() {
        items.push(BrowserInfo { name, path });
    }
}
...
fn find_browser() -> Result<BrowserInfo, String> {
    browser_candidates().into_iter().next().ok_or_else(|| {
        "Nenhum navegador Chromium compatível foi encontrado. Instale Microsoft Edge, Google Chrome ou Brave."
            .into()
    })
}
...
fn profile_data_dir(app: &tauri::AppHandle, profile_id: &str) -> Result<PathBuf, String> {
    let root = app
        .path()
        .app_data_dir()
        .map_err(|e| format!("Não foi possível localizar os dados do aplicativo: {e}"))?;
    let path = root.join("profiles").join(profile_id).join("browser-data");
    std::fs::create_dir_all(&path)
        .map_err(|e| format!("Não foi possível criar os dados do perfil: {e}"))?;
    Ok(path)
}

fn stop_process(mut process: BrowserProcess) {
    if let Some(stop) = process.bridge_stop.take() {
...
async fn launch_profile(
...
) -> Result<LaunchProfileResult, String> {
...

    {
        let mut processes = state
            .processes
            .lock()
            .map_err(|_| "Estado interno indisponível.")?;
        if let Some(existing) = processes.get_mut(&request.profile_id) {
            if existing
                ...
                .is_none()
            {
                return Err("Este perfil já está em execução.".into());
            }
            if let Some(old) = processes.remove(&request.profile_id) {
                if let Some(stop) = old.bridge_stop {
                    stop.store(true, Ordering::Relaxed);
                }
            }
        }
    ...
    if let Some(proxy) = request.proxy.clone() {
        if request.verify_ip || request.block_direct_fallback {
            ...
            if !test.ok {
                return Err(test.error.unwrap_or_else(|| {
                    "A proxy não passou na verificação de conectividade.".into()
                }));
            }
            proxy_ip = test.ip;
        }
        ...
        match proxy.protocol.as_str() {
            ...
            "http" | "https" => {
                ...
                if proxy.username.as_deref().is_some_and(|v| !v.is_empty()) {
                    warning = Some(
                        "O navegador pode solicitar usuário e senha da proxy HTTP/HTTPS na primeira conexão."
                            .into(),
                    );
                }
            }
    ...
    let child = command
        .spawn()
        .map_err(|e| format!("Não foi possível abrir {}: {e}", browser.name))?;
    let pid = child.id();
    let mut processes = state
        .processes
        .lock()
...
fn profile_status(
    ...
    let mut processes = state
        ...
        .lock()
        .map_err(|_| "Estado interno indisponível.")?;
    let Some(process) = processes.get_mut(&profile_id) else {
        return Ok(false);
    };
    match process.child.try_wait().map_err(|e| e.to_string())? {
        None => Ok(true),
        Some(_) => {
            if let Some(mut ended) = processes.remove(&profile_id) {
                if let Some(stop) = ended.bridge_stop.take() {
                    stop.store(true, Ordering::Relaxed);
                }
            }
            Ok(false)
...
mod tests {
    use super::{
        parse_host_port, validate_profile_id, validate_proxy_request, validated_start_url,
        ProxyTestRequest,
    };

    #[test]
    fn accepts_supported_proxy() {
        let request = ProxyTestRequest {
