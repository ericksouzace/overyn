...
export function SettingsPage({ profiles, proxies, security, onReset }: SettingsPageProps) {
  const exportData = () => {
    ...
    anchor.href = href;
    anchor.download = 'overyn-config.json';
    anchor.click();
  ...
  return (
    <main className="page">
      ...
      <section className="settings-card danger-zone"><div><span className="eyebrow">DADOS LOCAIS</span><h2>Restaurar dados de demonstração</h2><p>Apaga as alterações locais do painel e volta ao estado inicial.</p></div><button className="danger-outline-button" onClick={onReset}><RotateCcw size={16} /> Restaurar</button></section>
