...
export function ProfileModal({ open, profile, proxies, onClose, onSave }: ProfileModalProps) {
  ...
  useEffect(() => { if (!open) return; setName(profile?.name ?? ''); setNote(profile?.note ?? ''); setProxyId(profile?.proxyId ?? ''); }, [open, profile]);
  if (!open) return null;
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
      <section className="modal-card" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <p className="form-hint">Cookies, cache e armazenamento pertencem ao perfil. A configuração de rede é vinculada separadamente.</p>
        <div className="modal-actions"><button className="secondary-button" onClick={onClose}>Cancelar</button><button className="primary-button" disabled={name.trim().length < 2} onClick={() => onSave({ id: profile?.id ?? crypto.randomUUID(), name: name.trim(), note: note.trim(), proxyId: proxyId || undefined, status: profile?.status ?? 'ready', createdAt: profile?.createdAt ?? 'Agora' })}>{profile ? 'Salvar alterações' : 'Criar perfil'}</button></div>
      </section>
    </div>
  );
}
