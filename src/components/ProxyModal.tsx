...
export function ProxyModal({ open, proxy, onClose, onSave }: ProxyModalProps) {
  ...
  useEffect(() => {
    if (!open) return;
    setName(proxy?.name ?? ''); setProtocol(proxy?.protocol ?? 'socks5'); setHost(proxy?.host ?? ''); setPort(proxy ? String(proxy.port) : ''); setUsername(proxy?.username ?? ''); setPassword(proxy?.password ?? '');
  }, [open, proxy]);
  if (!open) return null;
  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
      <section className="modal-card wide" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="protocol-tabs" aria-label="Protocolo">{(['http', 'https', 'socks5'] as ProxyProtocol[]).map((item) => <button type="button" key={item} className={protocol === item ? 'active' : ''} onClick={() => setProtocol(item)}>{item.toUpperCase()}</button>)}</div>
        ...
        <p className="form-hint">A senha fica somente na memória da sessão atual e não é gravada no armazenamento local. SOCKS5 com autenticação usa uma ponte local do Overyn; HTTP/HTTPS pode exibir o prompt de autenticação do navegador.</p>
